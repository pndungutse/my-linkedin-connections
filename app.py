# -*- coding: utf-8 -*-
"""
Created on Fri Jan  6 11:37:13 2023
...
app.title = 'LinkedIn connections app'
server = app.server

# Import App data from csv sheets **************************************
df_cnt = pd.read_csv("https://raw.githubusercontent.com/pndungutse/data_linkedin/main/linkedin_data/Connections.csv", skiprows=2)
df_cnt["Connected On"] = pd.to_datetime(df_cnt["Connected On"])
df_cnt["month"] = df_cnt["Connected On"].dt.month
df_cnt["year"] = df_cnt["Connected On"].dt.year
df_cnt['month'] = df_cnt['month'].apply(lambda x: calendar.month_abbr[x])

df_invite = pd.read_csv("https://raw.githubusercontent.com/pndungutse/data_linkedin/main/linkedin_data/Invitations.csv")
...
df_msg = pd.read_csv("https://raw.githubusercontent.com/pndungutse/data_linkedin/main/linkedin_data/messages.csv")
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            ...
            dbc.Card([
                dbc.CardBody([
                    dbc.CardLink("Patrick Ndungutse", target="_blank", href="https://www.linkedin.com/in/patrick-ndungutse-571965152/", style={'textDecoration':'None'})
                    ])
                ]),
            ], width = 2),
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    html.H6("Connections"),
                    html.H2(id='content-connections-2', children="000")
                    ], style={'textAlign':'center'})
                ])
            ], width = 2)
...
@app.callback(
    ...
    Output('content-connections-2','children'),
...
def update_small_cards(start_date, end_date):
    ...
    return conctns_num, compns_num, in_num, out_num, reactns_num, conctns_num
...
#     # dff = dff.(dff['Connected On']>=start_date) & (dff['Connected On']<=end_date)
#     dff = dff[(dff['DATE']>=start_date) & (dff['DATE']<=end_date)]
#     dff = dff[["Position"]].value_counts()
#     # print(dff.head())
#     dff = dff.to_frame()
#     dff.reset_index(inplace=True)
#     dff.rename(columns={0:'Total in position'}, inplace=True)
#     dff



#     fig = px.scatter(dff,
#                      x="Position", y="Total in position", size="Total in position", color="Position",
#                      log_x=True, size_max=60,
#                      template="plotly", title="Gapminder 2007: theme")
